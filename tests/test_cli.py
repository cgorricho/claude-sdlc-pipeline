"""Tests for the csdlc CLI entry point."""
...
from claude_sdlc.cli import main
...
class TestInit:
    def test_init_non_interactive_creates_config(self, runner, tmp_path):
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            config_path = Path(".csdlc/config.yaml")
            assert config_path.exists()
            content = config_path.read_text()
            assert "pytest" in content

            assert Path(".csdlc/runs").is_dir()
    ...
    def test_init_non_interactive_node_detection(self, runner, tmp_path):
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            content = Path(".csdlc/config.yaml").read_text()
    ...
    def test_init_creates_gitignore(self, runner, tmp_path):
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            assert ".csdlc/runs/" in gitignore.read_text()
    ...
    def test_init_non_interactive_refuses_overwrite(self, runner, tmp_path):
        with runner.isolated_filesystem(temp_dir=tmp_path):
            Path(".csdlc").mkdir()
            Path(".csdlc/config.yaml").write_text("existing: true\n")
