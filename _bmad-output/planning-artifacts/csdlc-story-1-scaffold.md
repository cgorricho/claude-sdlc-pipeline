...
The story automation pipeline lives at `/home/cgorricho/apps/who_else_is_here/automation/` (7 source files, 9 test files, ~5,167 total lines). It needs to be physically moved into the standalone repo at `/home/cgorricho/apps/claude-sdlc-pipeline/` and restructured into a proper pip-installable Python package layout.

### Solution

Copy the existing files into `src/claude_sdlc/` package layout, create `pyproject.toml`, rename `auto_story.py` to `orchestrator.py`, and set up GitHub Actions CI. This is a **file move + rename + packaging** story — no logic changes.
...
- Verify `pip install -e .` works and `csdlc --help` prints (stub CLI is OK)
...
| Source (who_else_is_here) | Target (claude-sdlc-pipeline) | Notes |
|---|---|---|
| `automation/auto_story.py` (1255 lines) | `src/claude_sdlc/orchestrator.py` | Rename only |
| `automation/config.py` (111 lines) | `src/claude_sdlc/config.py` | Copy as-is |
| `automation/contracts.py` (219 lines) | `src/claude_sdlc/contracts.py` | Copy as-is |
| `automation/prompts.py` (372 lines) | `src/claude_sdlc/prompts.py` | Copy as-is |
| `automation/run_log.py` (301 lines) | `src/claude_sdlc/run_log.py` | Copy as-is |
| `automation/runner.py` (380 lines) | `src/claude_sdlc/runner.py` | Copy as-is |
| `automation/state.py` (104 lines) | `src/claude_sdlc/state.py` | Copy as-is |
...
| (none) | `src/claude_sdlc/__init__.py` | NEW: version + public API |
| (none) | `src/claude_sdlc/cli.py` | NEW: stub CLI (just --help) |
| (none) | `src/claude_sdlc/plugins.py` | NEW: empty placeholder |
| (none) | `src/claude_sdlc/plugins/__init__.py` | NEW: empty placeholder |
...
1. Copy all 7 source files from `/home/cgorricho/apps/who_else_is_here/automation/*.py` into `src/claude_sdlc/`, renaming `auto_story.py` to `orchestrator.py`
...
3. Fix import paths in ALL copied files: `from config import` → `from claude_sdlc.config import`, `from contracts import` → `from claude_sdlc.contracts import`, etc. Every internal import must use the `claude_sdlc.` package prefix. In test files, `from automation.X import` → `from claude_sdlc.X import`
4. Create `src/claude_sdlc/__init__.py` with `__version__ = "0.1.0"`
5. Create `src/claude_sdlc/cli.py` with a stub click CLI that prints help (just `@click.group` with `--help`)
6. Create `pyproject.toml` per Section 4 of the tech spec at `_bmad-output/planning-artifacts/claude-sdlc-pipeline-tech-spec.md`
7. Create `.github/workflows/ci.yml` running `pytest` and `ruff check` on push
8. Create empty placeholder files: `src/claude_sdlc/plugins.py`, `src/claude_sdlc/plugins/__init__.py`, `tests/__init__.py`
9. Add `LICENSE` file (MIT)
10. Verify `pip install -e .` succeeds
11. Verify `csdlc --help` prints usage
...
**AC-1**: All 7 source files from `/home/cgorricho/apps/who_else_is_here/automation/` are present under `src/claude_sdlc/` with correct names (auto_story.py renamed to orchestrator.py)

**AC-2**: All 8 test files from `/home/cgorricho/apps/who_else_is_here/automation/tests/` are present under `tests/`

**AC-3**: All internal imports updated to use `claude_sdlc.` package prefix — no bare `from config import`, `from contracts import`, etc.

**AC-4**: `pip install -e .` succeeds and `csdlc --help` prints usage
...
- Master tech spec: `_bmad-output/planning-artifacts/claude-sdlc-pipeline-tech-spec.md` (Sections 3, 4)
