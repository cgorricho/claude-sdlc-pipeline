...
**Approach:** Replace the flat constants module with a `Config` frozen dataclass loaded from `.bmpipe/config.yaml`. All access via `get_config()` singleton. Temporarily re-export old constant names as module-level aliases so existing consumers (Stories 4/6 will refactor them) keep working.
...
- Implement `bmpipe init` or `bmpipe validate` (Story 3)
...
|----------|--------------|---------------------------|----------------|
| Valid config | Well-formed `.bmpipe/config.yaml` | Frozen `Config` instance with all fields populated | N/A |
| Missing file | No `.bmpipe/config.yaml` | N/A | `FileNotFoundError` with message naming the expected path |
...
- `.bmpipe/config.yaml` -- NEW: project config matching tech spec Section 5 schema
...
- [x] `.bmpipe/config.yaml` -- Create project config pre-filled for bmad-sdlc itself (pytest commands, Python paths, default models)
...
**Acceptance Criteria:**
- Given a valid `.bmpipe/config.yaml`, when `get_config()` is called, then a frozen `Config` instance is returned with all fields correctly populated from YAML
...
**Interpolation**: Process all string values in `paths.*` and `test.reporter_args` after YAML parse. `{project_root}` resolves to the parent of `.bmpipe/` (i.e., the directory containing the config file's parent). `{runs_dir}` resolves to the resolved `paths.runs` value.

**Singleton**: `get_config()` uses a module-level `_config_instance: Config | None`. First call loads from `.bmpipe/config.yaml` relative to `PROJECT_ROOT` (determined by `Path(__file__).parent.parent.parent` — the repo root from `src/bmad_sdlc/config.py`). Provide `_reset_config()` for test isolation.
...
- `python -c "from bmad_sdlc.config import get_config; c = get_config(); print(c.project)"` -- expected: prints project config from `.bmpipe/config.yaml`
...
- Project YAML config pre-filled for bmad-sdlc
  [`config.yaml:1`](../../.bmpipe/config.yaml#L1)
