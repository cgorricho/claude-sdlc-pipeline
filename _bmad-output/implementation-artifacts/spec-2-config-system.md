...
**Problem:** `src/claude_sdlc/config.py` is a flat module with ~30 hardcoded constants (paths, binaries, models, timeouts, workflows, keyword maps). This makes the pipeline project-specific and non-reusable.

**Approach:** Replace the flat constants module with a `Config` frozen dataclass loaded from `.csdlc/config.yaml`. All access via `get_config()` singleton. Temporarily re-export old constant names as module-level aliases so existing consumers (Stories 4/6 will refactor them) keep working.
...
- Implement `csdlc init` or `csdlc validate` (Story 3)
...
|----------|--------------|---------------------------|----------------|
| Valid config | Well-formed `.csdlc/config.yaml` | Frozen `Config` instance with all fields populated | N/A |
| Missing file | No `.csdlc/config.yaml` | N/A | `FileNotFoundError` with message naming the expected path |
...
- `src/claude_sdlc/config.py` -- FULL REWRITE: Config dataclass + YAML loader + `get_config()` + backward-compat aliases
- `.csdlc/config.yaml` -- NEW: project config matching tech spec Section 5 schema
- `tests/test_config.py` -- NEW: unit tests for loading, validation, interpolation, keyword merge

## Tasks & Acceptance

**Execution:**
- [x] `src/claude_sdlc/config.py` -- Define nested frozen dataclasses (`PathsConfig`, `ModelsConfig`, `ClaudeConfig`, `CodexConfig`, `BuildConfig`, `TestConfig`, `TimeoutsConfig`, `ReviewConfig`, `SafetyConfig`, `StoryConfig`, `Config`). Implement `load_config(path: Path) -> Config` with YAML parsing, validation, `{project_root}`/`{runs_dir}` interpolation, and inference keyword merge. Implement `get_config() -> Config` singleton. Hardcode `_BUILTIN_INFERENCE_KEYWORDS`, `MODE_B_TAGS`, `STEP_MODES` as class constants. Re-export all current constant names as module-level aliases.
- [x] `.csdlc/config.yaml` -- Create project config pre-filled for claude-sdlc-pipeline itself (pytest commands, Python paths, default models)
...
**Acceptance Criteria:**
- Given a valid `.csdlc/config.yaml`, when `get_config()` is called, then a frozen `Config` instance is returned with all fields correctly populated from YAML
...
**Interpolation**: Process all string values in `paths.*` and `test.reporter_args` after YAML parse. `{project_root}` resolves to the parent of `.csdlc/` (i.e., the directory containing the config file's parent). `{runs_dir}` resolves to the resolved `paths.runs` value.

**Singleton**: `get_config()` uses a module-level `_config_instance: Config | None`. First call loads from `.csdlc/config.yaml` relative to `PROJECT_ROOT` (determined by `Path(__file__).parent.parent.parent` — the repo root from `src/claude_sdlc/config.py`). Provide `_reset_config()` for test isolation.
...
- `python -c "from claude_sdlc.config import get_config; c = get_config(); print(c.project)"` -- expected: prints project config from `.csdlc/config.yaml`
- `python -c "from claude_sdlc.config import SPRINT_STATUS, CLAUDE_BIN; print(SPRINT_STATUS, CLAUDE_BIN)"` -- expected: prints default values (backward compat aliases work)
- `ruff check src/claude_sdlc/config.py` -- expected: passes
