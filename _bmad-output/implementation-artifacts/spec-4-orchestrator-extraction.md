...
- `src/claude_sdlc/orchestrator.py` -- MODIFY: Replace all 13 config constant imports with `get_config()`, delete `run_schema_drift_check()` and its caller, rewrite `glob_implementation_files()`
- `src/claude_sdlc/config.py` -- MODIFY: Add `source_dirs: list[str]` and `exclude_patterns: list[str]` to `ProjectConfig`
- `tests/test_orchestrator.py` -- MODIFY: Mock `get_config()` instead of individual constants, remove schema drift tests

## Tasks & Acceptance

**Execution:**
- [x] `src/claude_sdlc/config.py` -- Add `source_dirs: list[str] = field(default_factory=list)` and `exclude_patterns: list[str] = field(default_factory=lambda: ["node_modules", "dist", ".next", ".turbo"])` to `ProjectConfig`. Add both to `_KNOWN_TOP_KEYS` validation if nested key checking applies.
- [x] `src/claude_sdlc/orchestrator.py` -- Replace import block (lines 35-40): single `from claude_sdlc.config import get_config, Config`. Remove `get_review_step_mode` import.
- [x] `src/claude_sdlc/orchestrator.py` -- In `run_pipeline()`: add `config = get_config()` and local Path vars (`project_root`, `sprint_status`, `impl_artifacts`, `runs_dir`, `test_artifacts`). Replace all ~50 constant references in `run_pipeline()` body with config/local var equivalents.
- [x] `src/claude_sdlc/orchestrator.py` -- Refactor helper functions (`_scoped_clean`, `should_run_step`, `determine_resume_step`, `find_latest_run`, `log_step_skip`, `fail_step`, `glob_implementation_files`, `_find_findings_file`, `parse_review_findings`, `apply_safety_heuristic`, `generate_escalation_doc`) to accept config or derived values as parameters instead of using module-level constants.
- [x] `src/claude_sdlc/orchestrator.py` -- Inline `get_review_step_mode()` at its single call site (line 341) as `config.STEP_MODES[f"code-review-mode-{run_log.review_mode.lower()}"]`.
- [x] `src/claude_sdlc/orchestrator.py` -- Rewrite `glob_implementation_files()` to use `config.project.source_dirs` for directory filtering and `config.project.exclude_patterns` for exclusions. When `source_dirs` is empty, return all changed files without dir filter. Remove hardcoded `packages/` fallback glob.
- [x] `src/claude_sdlc/orchestrator.py` -- Delete `run_schema_drift_check()` (lines 1129-1181) and its logger `_drift_log`. Replace caller (lines 292-298) with comment `# Plugin hook: pre_review_checks (see Story 5)`.
- [x] `src/claude_sdlc/orchestrator.py` -- Replace hardcoded trace report path (line 819) `PROJECT_ROOT / "_bmad-output/test-artifacts"` with `Path(config.paths.test_artifacts)`.
...
- Given a valid `.csdlc/config.yaml`, when `run_pipeline("1-3", dry_run=True)` executes, then it uses config values (not hardcoded defaults) for model names and step listing
...
- `ruff check src/claude_sdlc/orchestrator.py` -- expected: no lint errors
- `grep -c 'from claude_sdlc.config import' src/claude_sdlc/orchestrator.py` -- expected: 1 (only `get_config, Config`)
- `grep -c '_bmad-output' src/claude_sdlc/orchestrator.py` -- expected: 0
- `grep -c 'run_schema_drift_check' src/claude_sdlc/orchestrator.py` -- expected: 0
- `grep -c 'packages/' src/claude_sdlc/orchestrator.py` -- expected: 0
