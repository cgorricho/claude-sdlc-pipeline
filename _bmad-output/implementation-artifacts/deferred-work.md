...
- **Stale command references in docstrings/messages**: 15+ references to `python automation/auto_story.py` in `orchestrator.py`, `prompts.py`, and `config.py` docstrings/user messages. Should be updated to `bsdlc run` when Story 4 refactors the orchestrator.
...
- **Stale `auto_story.py` docstring in orchestrator.py**: Module docstring (lines 1-23) still references `auto_story.py` and `python automation/auto_story.py --story 1-3`. Should be updated to reference `bsdlc run` when Story 4 refactors the orchestrator.
