...
**Approach:** Copy all source and test files into `src/claude_sdlc/` package layout, rewrite imports to use the `claude_sdlc.` package prefix, create `pyproject.toml` with a `csdlc` console script entry point, and set up GitHub Actions CI. No logic changes — this is purely structural.

## Boundaries & Constraints

**Always:**
- Preserve original file contents exactly (only change imports and the filename `auto_story.py` → `orchestrator.py`)
- Use `src/` layout (`src/claude_sdlc/`) per tech spec Section 3
...
- `src/claude_sdlc/` -- target package directory (new)
...
**Execution:**
- [x] `src/claude_sdlc/` -- copy 7 source files from `who_else_is_here/automation/`, renaming `auto_story.py` → `orchestrator.py`
- [x] `tests/` -- copy 8 test files + `conftest.py` from `who_else_is_here/automation/tests/`
- [x] `src/claude_sdlc/**/*.py`, `tests/**/*.py` -- rewrite all bare imports (`from config import` → `from claude_sdlc.config import`, etc.) and update `auto_story` references to `orchestrator`
- [x] `src/claude_sdlc/__init__.py` -- create with `__version__ = "0.1.0"`
- [x] `src/claude_sdlc/cli.py` -- create stub click CLI (`@click.group` with `--help` only)
- [x] `pyproject.toml` -- create per tech spec Section 4 (name, deps, console script)
- [x] `src/claude_sdlc/plugins.py`, `src/claude_sdlc/plugins/__init__.py`, `tests/__init__.py` -- create empty placeholders
- [x] `.github/workflows/ci.yml` -- pytest + ruff on push
- [x] `LICENSE` -- MIT license file

**Acceptance Criteria:**
- Given the package is installed with `pip install -e .`, when running `csdlc --help`, then usage text is printed
- Given `src/claude_sdlc/` exists, when running `grep -r "who_else_is_here\|whoelseishere" src/`, then zero matches are returned
- Given all files are copied, when running `grep -rn "^from config import\|^from contracts import\|^from prompts import\|^from run_log import\|^from runner import\|^from state import\|^from auto_story import" src/ tests/`, then zero matches (all imports use `claude_sdlc.` prefix)
...
- `csdlc --help` -- expected: prints click help text
...
- Version and public API surface
  [`__init__.py:1`](../../src/claude_sdlc/__init__.py#L1)

- Stub CLI wired as `csdlc` — confirms entry point works
  [`cli.py:1`](../../src/claude_sdlc/cli.py#L1)

**Import rewriting (the core change)**

- Heaviest file — 6 import blocks rewritten from bare to `claude_sdlc.` prefix
  [`orchestrator.py:36`](../../src/claude_sdlc/orchestrator.py#L36)

- Logger names updated from `"auto_story"` to package-qualified names
  [`orchestrator.py:184`](../../src/claude_sdlc/orchestrator.py#L184)

- Inline imports in function bodies — easy to miss
  [`contracts.py:70`](../../src/claude_sdlc/contracts.py#L70)

- Test file with most patch() string rewrites (`auto_story.` → `claude_sdlc.orchestrator.`)
